...
fn list_windows() -> Vec<X11Window> {
	...
	for i in 0..num_children {
		let window_id = unsafe { *children.offset(i as isize) };
		let mut attributes: xlib::XWindowAttributes = unsafe { std::mem::zeroed() };
		unsafe {
			xlib::XGetWindowAttributes(screen.display_ptr, window_id, &mut attributes);
		}

		if attributes.map_state == xlib::IsViewable {
			let window = X11Window::new(window_id).populate(&screen);
	...
	unsafe {
		...
		xlib::XCloseDisplay(screen.display_ptr);
	}
	x11windows
}
